 ...
 */
/*
 * NOTICE : This file has been modified since its original version.
 * Modification by : JŽr™me Monceaux (bilock@gmail.com) 
 */
...
/**
 * Implements communication via XMPP.
 * 
...
public class XmppCommChannel extends AbstractCommChannel {
    ...
    private static final String SERVICE = "xmpp.aldebaran-robotics.com";
    ...
    @Override
    public void connect() {
    	Log.i("XmppCommChannel", "connect");
        // Create a connection
        mConnection = new XMPPConnection(SERVICE);
        listenForMessages(0, false); 
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                	Log.i("XmppCommChannel", "connect run");
                    mConnection.connect();
    ...
    public void send(String to, String message) {
        ...
        */
        if (mConnection == null)
            return;
        //Message msg = new Message(to + "@" + SERVICE /*+ "/nao"*/, Message.Type.chat);
        
        Message msg = new Message(to, Message.Type.chat);
        msg.setBody(message);
        //Log.i("XmppCommChannel.java",String.format("sending to : %s body : %s", msg.getTo(), limitString(100,msg.getBody())));
    ...
    public void listenForMessages(final long waitTimeBetweenPolling, boolean returnStream) {
        ...
        mConnection.addPacketListener(new PacketListener() {
            public void processPacket(Packet packet) {
                ...
                Log.i("XmppCommChannel", String.format("processPacket from %s", message.getFrom()));          	
                //Log.i("XmppCommChannel", String.format("processPacket from %s : %s", message.getFrom(), limitString(message.getBody())));
            	if( message.getFrom().contains("Smack") )
            		return;
